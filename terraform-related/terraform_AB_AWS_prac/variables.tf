...
variable "aws_region" {
  ...
  type        = string
  default     = "ap-northeast-2" # 원하는 AWS 리전으로 변경 (예: 서울)
}

variable "project_name" {
  description = "Name of the project, used for resource naming and tags"
  type        = string
  default     = "llm-project" # 프로젝트 이름
}

variable "aws_tags" {
  description = "Map of tags to apply to all taggable AWS resources"
  type        = map(string)
  default = {
    Project     = "LLM Inhouse"
    Environment = "Dev" # 또는 Prod 등
    ManagedBy   = "Terraform"
  }
}
...
variable "public_subnets_cidr" {
  ...
  type        = list(string)
  # 원하는 가용 영역(AZ)과 Subnet CIDR로 변경
...
variable "eks_cluster_version" {
  ...
  type        = string
  default     = "1.28" # 사용하려는 K8s 버전으로 변경 (AWS EKS 지원 버전 확인)
}

variable "eks_node_group_instance_type" {
  description = "EC2 instance type for the EKS node group"
  type        = string
  default     = "t3.medium" # 원하는 인스턴스 타입으로 변경 (비용 고려)
}
...
variable "eks_node_group_max_size" {
  ...
  type        = number
  default     = 3 # 오토스케일링 최대치
}

variable "eks_node_group_min_size" {
  description = "Minimum number of nodes in the EKS node group"
  type        = number
  default     = 1 # 오토스케일링 최소치
}

# --- OpenWebUI A/B Testing Configuration ---
variable "openwebui_v1_weight" {
  description = "Traffic weight percentage for OpenWebUI v1 (0-100)"
  type        = number
  default     = 50 # 기본 50%
}

variable "openwebui_v2_weight" {
  description = "Traffic weight percentage for OpenWebUI v2 (0-100)"
  type        = number
  default     = 50 # 기본 50%
  # Note: v1_weight + v2_weight should ideally equal 100
}
